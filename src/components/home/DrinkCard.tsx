...
// --- Part 4: Timings & Easing (from the spec) ---
const generalEase = [0.25, 0.1, 0.25, 1];
const heroPopEase = [0.22, 1, 0.36, 1];

const generalTransition = { duration: 0.26, ease: generalEase };
const popTransition = { duration: 0.26, ease: heroPopEase };
const revealTransition = { duration: 0.28 };
...
export const DrinkCard = ({ drink, onClick, isSelected }: DrinkCardProps) => {
  ...
  // Variants for users who prefer reduced motion
  const reducedMotionVariants = {
    rest: { opacity: 1 },
    inView: { opacity: 1 },
    reveal: { opacity: 0 },
  };
  const reducedIngredientsVariants = {
    rest: { opacity: 0 },
    inView: { opacity: 0 },
    reveal: { opacity: 1 },
  };
